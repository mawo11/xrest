...
internal static class FacebookEndpoint
{
	...
	{
		applicationBuilder.MapGet("/authenticate/facebook", async (
			HttpRequest request,
			HttpResponse response,
			[FromServices] ILogger<Object> logger,
			IConfiguration configuration,
			IExternalAuthenticateService externalAuthenticateService,
			ClientType clientType) =>
	   {
			   ...
			   || string.IsNullOrEmpty(authResult.Properties.GetTokenValue("access_token")))
		   {
			   ...
			   await request.HttpContext.ChallengeAsync(authScheme, new AuthenticationProperties
			   {
				   ...
				   RedirectUri = $"{externalAuthenticateAppUrl.TrimEnd('/')}/authenticate/facebook?clienttype={(int)clientType}"
			   });
		   ...
		   var result = await externalAuthenticateService.Login(new Identity.Contracts.ExternalAuthenticate.Request.ExternalAuthenticateLogonRequest
		   {
			   ...
			   Surname = surname,
			   ClientType = clientType,
			   ExternalProvider = ExternalProviderType.Facebook
		   });
